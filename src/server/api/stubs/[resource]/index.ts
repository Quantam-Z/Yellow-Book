...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (method === 'GET') {
      const response = await executeStubRequest({ resource, method: 'GET', delay });
      return respondWithSuccess(event, response);
    }

    if (method === 'DELETE') {
      const response = await executeStubRequest({ resource, method: 'DELETE', delay });
      return respondWithSuccess(event, response);
    }

    if (METHODS_WITH_BODY.has(method)) {
      const payload = await readBody(event);
      const response = await executeStubRequest({ resource, method, payload, delay });
      return respondWithSuccess(event, response);
    }
