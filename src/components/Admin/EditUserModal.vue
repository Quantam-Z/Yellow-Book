  ...
  <Transition name="edit-modal-fade">
    ...
    >
      <div class="relative w-full max-w-lg rounded-2xl bg-white shadow-2xl p-6 sm:p-7 font-plus-jakarta-sans">
        ...
        <div class="mb-6">
          <h3 class="text-xl font-semibold text-gray-900 leading-tight">Edit User</h3>
          <p class="text-sm text-gray-500 mt-1">
            Update user details. Changes take effect immediately.
          </p>
        </div>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <label class="text-sm font-medium text-gray-700">Signup method</label>
              <select
                v-model="form.signupMethod"
              ...
              >
                <option v-for="method in signupMethodOptions" :key="method" :value="method">
                  {{ method }}
                </option>
              </select>
            </div>
...
const DEFAULT_STATUSES = ['Active', 'Inactive', 'Suspended', 'Pending']
const DEFAULT_SIGNUP_METHODS = ['Email', 'Google', 'Facebook', 'Twitter', 'LinkedIn']

const props = defineProps({
  open: { type: Boolean, default: false },
  user: { type: Object as () => Record<string, any> | null, default: null },
  statuses: { type: Array as () => string[], default: () => [] },
  signupMethods: { type: Array as () => string[], default: () => [] },
})
...
const signupMethodOptions = computed(() =>
  props.signupMethods?.length ? props.signupMethods : DEFAULT_SIGNUP_METHODS
)

const createFormState = (user: Record<string, any> | null) => ({
  name: user?.name ?? '',
  email: user?.email ?? '',
  status: statusOptions.value.includes(user?.status)
    ? user?.status
    : statusOptions.value[0],
  signupMethod: signupMethodOptions.value.includes(user?.signupMethod)
    ? user?.signupMethod
    : signupMethodOptions.value[0],
  verified: Boolean(user?.verified),
})

const form = ref(createFormState(props.user))

watch(
  [() => props.user, statusOptions, signupMethodOptions],
  () => {
    form.value = createFormState(props.user)
  },
  { immediate: true }
)

const canSubmit = computed(() =>
  Boolean(props.user?.id && form.value.name.trim() && form.value.email.trim())
)
...
const handleSubmit = async () => {
  if (!props.user?.id || submitting.value) return
  ...
  const payload = {
    ...
    signupMethod: form.value.signupMethod,
    verified: form.value.verified,
  }

  try {
    await stubClient.update('users', props.user.id, payload, { delay: 200 })
