<template>
  <div class="flex">
    ...
    <!-- Sidebar -->
    <aside
      class="w-[280px] min-h-screen bg-white border-r border-[#eee] shadow-[4px_12px_23px_rgba(0,0,0,0.08)] 
             py-6 px-4 flex flex-col fixed md:static top-0 left-0 h-full z-[52]
      ...
      <div class="flex-1 flex flex-col overflow-y-auto pr-1">
        <nav class="flex flex-col gap-2">
          <NuxtLink
            ...
            :key="index"
            :to="item.to"
            class="flex items-center gap-3 py-3 px-4 rounded-lg text-base font-medium text-[#212121]
                   transition-all duration-200 ease-in-out hover:bg-[#fafafa] no-underline"
...
useClientEventListener(() => window, "scroll", handleScroll, {
  passive: true,
  immediate: true,
});
</script>
