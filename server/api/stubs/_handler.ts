...
export const createStubEventHandler = (options: StubRouteOptions = {}) => {
  const { idParam } = options;

  return defineEventHandler(async (event) => {
    const resource = event.context.params?.resource;
    const id = idParam ? event.context.params?.[idParam] : undefined;
...
export default createStubEventHandler;
